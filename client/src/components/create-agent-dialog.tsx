...
function mkConn(agentId: string, src: string, tgt: string, handle: string, label: string): FlowConnection {
  return { id: makeId(), agentId, sourceNodeId: src, targetNodeId: tgt, sourceHandle: handle, label };
}

function buildStarterFlow(templateId: string, agentId: string): { nodes: FlowNode[]; connections: FlowConnection[] } {
  const spacing = 420;
  const y = 200;
  ...
  if (templateId === "reservation-agent") {
    ...
    return {
      ...
      connections: [
        mkConn(agentId, n0, n1, 'next', 'Continue'),
        mkConn(agentId, n1, n2, 'collected', 'Details Collected'),
        mkConn(agentId, n2, n3, 'next', 'Continue'),
      ],
    };
  }
  ...
  if (templateId === "order-taking-agent") {
    ...
    return {
      ...
      connections: [
        mkConn(agentId, n0, n1, 'next', 'Continue'),
        mkConn(agentId, n1, n2, 'collected', 'Order Taken'),
        mkConn(agentId, n2, n3, 'next', 'Continue'),
      ],
    };
  }
  ...
  if (templateId === "general-inquiries-agent") {
    ...
    return {
      ...
      connections: [
        mkConn(agentId, n0, n1, 'next', 'Continue'),
        mkConn(agentId, n1, n2, 'answered', 'Question Answered'),
      ],
    };
  }
  ...
  if (templateId === "catering-agent") {
    ...
    return {
      ...
      connections: [
        mkConn(agentId, n0, n1, 'next', 'Continue'),
        mkConn(agentId, n1, n2, 'collected', 'Details Collected'),
        mkConn(agentId, n2, n3, 'transferred', 'Transferred'),
      ],
    };
  }
...
export function CreateAgentDialog({ open, onOpenChange }: CreateAgentDialogProps) {
  ...
  const installMutation = useMutation({
    mutationFn: async (template: typeof RESTAURANT_TEMPLATES[0]) => {
      ...
      const { nodes: starterNodes, connections: starterConnections } = buildStarterFlow(template.id, agent.id);
      if (starterNodes.length > 0) {
        await apiRequest("POST", `/api/agents/${agent.id}/flow-nodes/bulk`, { nodes: starterNodes });
      }
      if (starterConnections.length > 0) {
        await apiRequest("POST", `/api/agents/${agent.id}/flow-connections/bulk`, { connections: starterConnections });
      }
    ...
    onSuccess: (agent) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/agents", agent.id, "flow-connections"] });
      toast({
