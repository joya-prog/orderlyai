...
- **Analytics Dashboard**: Premium dashboard with personalized time-based greeting, 4 KPI stat cards (Total Calls, Minutes Used, Estimated Cost, Avg Call Duration) each with mini sparklines, trend badges vs previous period, and expand dialogs. Charts include: grouped Call Volume bar chart, Revenue Trend area chart (from order_placed event amounts), Peak Call Hours horizontal bar chart (color-coded), AI Performance Score radial gauge (call-to-order conversion), and Recent Calls mini-list. Date range selector (7d/30d/90d/12mo). New endpoint `GET /api/analytics/usage-summary` returns current + previousPeriod stats from `usage_ledger` + `call_logs`. All panels expand to full-screen dialogs. Friendly empty states throughout.
